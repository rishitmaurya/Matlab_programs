% MATLAB Program to compute gradient magnitude and orientation using Prewitt filter (without built-in functions)
...
% Define Prewitt operator manually
Gx_kernel = [-1 0 1; -1 0 1; -1 0 1];  % Prewitt filter for X-direction
Gy_kernel = [-1 -1 -1; 0 0 0; 1 1 1];  % Prewitt filter for Y-direction
...
% Apply Prewitt filter manually (without imfilter)
for i = 2:row-1
    for j = 2:col-1
