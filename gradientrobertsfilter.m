% MATLAB Program to compute gradient magnitude and orientation using Roberts filter (without built-in functions)
...
% Define Roberts operator manually
Gx_kernel = [1 0; 0 -1];
Gy_kernel = [0 1; -1 0];
...
% Apply Roberts filter manually (without imfilter)
for i = 1:row-1
    for j = 1:col-1
        Gx(i, j) = sum(sum(Gx_kernel .* grayImg(i:i+1, j:j+1)));
        Gy(i, j) = sum(sum(Gy_kernel .* grayImg(i:i+1, j:j+1)));
    end
end
